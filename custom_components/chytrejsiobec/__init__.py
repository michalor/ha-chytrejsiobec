...
class ChytrejsiObecDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    async def _async_update_data(self):
        """Fetch data from API with retry on transient errors."""
        _LOGGER.info("ChytrejsiObec update cycle starting")
        ...
        for attempt in range(RETRY_LIMIT):
            try:
                _LOGGER.debug("Fetching devices from %s (attempt %d/%d)", API_URL, attempt + 1, RETRY_LIMIT)
                ...
                if attempt < RETRY_LIMIT - 1:
                    ...
                    _LOGGER.warning("Retrying after timeout in %.1f seconds", wait_seconds)
                    await asyncio.sleep(wait_seconds)
                else:
                    raise UpdateFailed(f"Timeout after {RETRY_LIMIT} attempts")
            except aiohttp.ClientResponseError as err:
                _LOGGER.error("Client response error: %s", err, exc_info=True)
                if attempt < RETRY_LIMIT - 1:
                    wait_seconds = RETRY_DELAY * (2 ** attempt)
                    _LOGGER.warning("Retrying after client error in %.1f seconds", wait_seconds)
                    await asyncio.sleep(wait_seconds)
                else:
                    raise UpdateFailed(f"Client response error after {RETRY_LIMIT} attempts: {err}")
            except aiohttp.ClientError as err:
                _LOGGER.error("Error communicating with API: %s", err, exc_info=True)
                if attempt < RETRY_LIMIT - 1:
                    wait_seconds = RETRY_DELAY * (2 ** attempt)
                    _LOGGER.warning("Retrying after network error in %.1f seconds", wait_seconds)
                    await asyncio.sleep(wait_seconds)
                else:
                    raise UpdateFailed(f"Network error after {RETRY_LIMIT} attempts: {err}")
            except UpdateFailed:
                ...
                if attempt < RETRY_LIMIT - 1:
                    wait_seconds = RETRY_DELAY * (2 ** attempt)
                    _LOGGER.warning("Retrying after unexpected error in %.1f seconds", wait_seconds)
                    await asyncio.sleep(wait_seconds)
                else:
                    raise UpdateFailed(f"Unexpected error after {RETRY_LIMIT} attempts: {err}")